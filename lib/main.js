            ...
            for (const category of categories.sort((a, b) => a.rawPosition - b.rawPosition)) {
                setTimeout(async () => {
                    for (const perms of category.permissionsOverwrites) {
                        if (perms.type === discord_js_1.OverwriteType.Role) {
                ...
                switch (order) {
                    case "channels_roles":
                        ...
                        this.createRoles(backup.roles, guild.roles, guild.members, 150).then(async (rolesCorrespondence) => {
                            ...
                            this.createCategories(backup.channels.filter((c) => c.type === discord_js_1.ChannelType.GuildCategory), guild.channels, rolesCorrespondence).then((categoriesCorrespondence) => {
                                // create channels
                                this.createChannels(backup.channels.filter((c) => c.type === discord_js_1.ChannelType.GuildText || c.type === discord_js_1.ChannelType.GuildNews || c.type === discord_js_1.ChannelType.GuildVoice), guild.channels, categoriesCorrespondence, rolesCorrespondence);
                            });
                        });
                        break;
                    case "create_emojis":
